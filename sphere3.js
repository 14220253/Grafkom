...
Sphere3.prototype =
{
    ...
    buildVerticesSmooth: function()
    {
        ...
        for(i=0; i <= this.stackCount; i += 1)
        {
            ...
            //hyperboloid 1 sisi
            // stackAngle = Math.PI / 2 - i * stackStep;   // starting from pi/2 to -pi/2
            // xy = this.radius * 1/Math.sin(stackAngle);    // r * cos(u)
            // z = this.radius * Math.tan(stackAngle);     // r * sin(u)

            //hyperboloid 2 sisi

            // if (this.is2Side == false) {
                // stackAngle = Math.PI / 2 - i * stackStep;   // starting from pi/2 to -pi/2
                // xy = this.radius * Math.sinh(stackAngle);    // r * cos(u)
                // z = this.radius * Math.cosh(stackAngle);     // r * sin(u)
            // } else { 
                // stackAngle = Math.PI / 2 - i * stackStep;   // starting from pi/2 to -pi/2
                // xy = this.radius * Math.sinh(stackAngle);    // r * cos(u)
                // z = -(this.radius * Math.cosh(stackAngle));     // r * sin(u)
            ...
            for(j=0; j <= this.sectorCount; ++j)
            {
                ...
                // y = this.radius * stackAngle *1/ Math.cos(sectorAngle);         // r * cos(u) * sin(v)
    ...
    buildInterleavedVertices: function()
    {
        ...
        let i, j, k;
        for(i=0, j=0, k=0; i < this.vertices.length; i+=3, j+=2, k+=6)
        {
            this.interleavedVertices[k]   = this.vertices[i] + this.posX;
            this.interleavedVertices[k+1] = this.vertices[i+1] + this.posY;
            this.interleavedVertices[k+2] = this.vertices[i+2] + this.posZ;
            this.interleavedVertices[k+3] = this.r;
            this.interleavedVertices[k+4] = this.g;
            this.interleavedVertices[k+5] = this.b;
