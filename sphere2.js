...
let Sphere2 = function(gl, radius=1, sectors=36, stacks=18, smooth=true, is2Side=false)
{
    this.gl = gl;
    if(!gl)
    ...
    // init
    this.set(radius, sectors, stacks, smooth, is2Side);
};

Sphere.prototype =
{
    set: function(r, se, st, sm, i2s)
    {
        this.is2Side = i2s;
        this.radius = r;
        this.sectorCount = se;
    ...
    buildVerticesSmooth: function()
    {
        ...
        for(i=0; i <= this.stackCount; ++i)
        {
            ...
            //hyperboloid 1 sisi
            stackAngle = Math.PI / 2 - i * stackStep;   // starting from pi/2 to -pi/2
            xy = this.radius * 1/Math.cos(stackAngle);    // r * cos(u)
